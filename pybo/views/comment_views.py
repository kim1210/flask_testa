...
def create_question(question_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        db.session.commit()
        #return redirect(url_for('question.detail', question_id=question_id)) 수정(p.226)
...
def modify_question(comment_id):
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            db.session.commit()
            #return redirect(url_for('question.detail', question_id=comment.question.id)) 수정(p.226)
            return redirect('{}#comment_{}'.format(url_for('question.detail', question_id=question_id), comment.id))
...
def create_answer(answer_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        db.session.add(comment)
        db.session.commit()
        #return redirect(url_for('question.detail', question_id = answer.question.id)) 수정(p.227)
...
def modify_answer(comment_id):
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            # 질문 댓글 함수와 거의 차이가 없지만, question_id 필드의 값을 알기 위해 comment.answer.question.id와 같이 answer를 사용해서 question에 접근하고 있다. 
